...
import { Derivable, Reactor } from '../types'
...
const unpack = (value: unknown): Letter => {
	if (value instanceof Derivation || value instanceof Atom) {
...
type FuzzSystemState = {
	...
	derivations: Record<
		string,
		{ derivation: Derivation<Letter>; sneakyGet: () => Letter }
	>
	derivationsInDerivations: Record<string, Derivation<Derivation<Letter>>>
	atomsInDerivations: Record<string, Derivation<Atom<Letter>>>
	reactors: Record<
		string,
		{ reactor: Reactor; result: string | null; dependencies: Derivable<any>[] }
	>
}
...
class Test {
	...
	unpack_sneaky = (value: unknown): Letter => {
		if (value instanceof Derivation) {
	...
	constructor(seed: number) {
		...
		times(this.source.nextIntInRange(1, MAX_ATOMS), () => {
			...
			this.systemState.atoms[atomId] = new Atom(
				atomId,
				this.source.selectOne(LETTERS)
			)
		})

		times(this.source.nextIntInRange(1, MAX_ATOMS_IN_ATOMS), () => {
			const atomId = this.source.nextId()
			this.systemState.atomsInAtoms[atomId] = new Atom(
				atomId,
		...
		times(this.source.nextIntInRange(1, MAX_ATOMS_IN_DERIVATIONS), () => {
			...
			this.systemState.atomsInDerivations[derivationId] = new Derivation(
				derivationId,
				() => atom,
				{
					isPush: this.source.choice(0.3),
				}
			)
		})
		...
		times(this.source.nextIntInRange(1, MAX_DERIVATIONS_IN_DERIVATIONS), () => {
			const derivationId = this.source.nextId()
			this.systemState.derivationsInDerivations[derivationId] = new Derivation(
				derivationId,
				() =>
		...
		times(this.source.nextIntInRange(1, MAX_REACTORS), () => {
			...
			const dependencies: Derivable<any>[] = []
			...
			times(this.source.nextIntInRange(1, MAX_DEPENDENCIES_PER_ATOM), () => {
				this.source.executeOne({
					...
					'add a random derivation': () => {
						dependencies.push(
							this.source.selectOne(
								Object.values(this.systemState.derivations).map(
									(d) => d.derivation
								)
							)
						)
					...
					'add a random derivation in derivation': () => {
						dependencies.push(
							this.source.selectOne(
								Object.values(this.systemState.derivationsInDerivations)
							)
						)
					},
			...
			this.systemState.reactors[reactorId] = {
				reactor: reactor(reactorId, () => {
					this.systemState.reactors[reactorId].result = dependencies
						.map(unpack)
						.join(':')
				}),
				result: '',
		...
		return this.source.executeOne<Op>({
			...
			'deref atom in derivation': () => {
				return {
					...
					id: this.source.selectOne(
						Object.keys(this.systemState.atomsInDerivations)
					),
				}
			},
			'deref derivation in derivation': () => {
				return {
					...
					id: this.source.selectOne(
						Object.keys(this.systemState.derivationsInDerivations)
					),
				}
			},
			'deref derivation': () => {
			...
			'run several ops in a transaction': () => {
				return {
					...
					ops: times(
						this.source.nextIntInRange(2, MAX_OPS_IN_TRANSACTION),
						() => this.getNextOp()
					),
				}
			},
	...
	applyOp(op: Op) {
		switch (op.type) {
			...
			case 'update_atom_in_atom': {
				this.systemState.atomsInAtoms[op.id].set(
					this.systemState.atoms[op.atomId]
				)
				break
			}
