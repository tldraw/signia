...
:::tip
If you are using `signia-react-jsx`, feel free to skip this section unless you're using Remix. Your components are already reactive! ✨
:::
...
:::tip Remix
Using `jsxImportSource` to set up reactivity may take some extra care with Remix.
...
3. At present, the updated pragma won't affect the components you export from your route modules via `export default`, but it will affect their children.

You have several options.

A. Rely on the 	`useValue` hook in your export default components and the pragma (`signia-react-jsx`) in their children.
B. Wrap each of your route's export default components in `track`, then rely on the pragma in their children.
C. Rely on the pragma by abstracting your export default component logic into a new component:
...
export default function MyRoute() {
	...
	const signiaState = SigniaClass.useSigniaState(data);
	const stringArray = signiaState.myStringsDataArray;
