...
export class HistoryBuffer<Diff> {
	...
	/**
	 * Create a history buffer, a structure that stores diffs between values of an
	 * atom.
	 *
	 ...
	 * @param diffThe Diff to add, or else a reset value.
	 * @public
	 */
	pushEntry(
		lastComputedEpoch: number,
		currentEpoch: number,
		diff: Diff | RESET_VALUE
	) {
		if (diff === undefined) {
			return
		}
	 ...
	 * @returns An array of diffs or else a reset value.
	 * @public
	 */
